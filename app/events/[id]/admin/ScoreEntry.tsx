...
function buildScoreSheetHtml(courts: Array<{ courtNumber: number; round1: MatchForPrint | null }>): string {
  const paddle = `<svg viewBox="-5 -5 75 90" width="28" height="38" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="0" y="0" width="38" height="50" rx="19" fill="none" stroke="#f5a623" stroke-width="3.5"/><line x1="19" y1="50" x2="19" y2="68" stroke="#f5a623" stroke-width="3.5" stroke-linecap="round"/><circle cx="50" cy="4" r="7" fill="#f5a623"/></svg>`

  function numBox(n: number | null | undefined, filled: boolean): string {
    const bg  = filled && n != null ? '#111' : 'transparent'
    const col = filled && n != null ? '#fff' : '#aaa'
    const bdr = filled && n != null ? '#111' : '#bbb'
    return `<div style="min-width:26px;height:26px;padding:0 4px;background:${bg};border:1.5px solid ${bdr};display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;flex-shrink:0;color:${col};">${n != null ? n : '#'}</div>`
  }

  function playerRow(n: number | null | undefined, name: string | null | undefined, side: 'left' | 'right', filled: boolean): string {
    const box  = numBox(n, filled)
    const line = `<div style="flex:1;border-bottom:1.5px solid #333;padding-bottom:2px;padding-left:${side === 'left' ? '4px' : '0'};padding-right:${side === 'right' ? '4px' : '0'};font-size:11px;color:#111;text-align:${side === 'right' ? 'right' : 'left'};min-width:60px;">${filled && name ? name : ''}</div>`
    const items = side === 'left' ? `${box}${line}` : `${line}${box}`
    return `<div style="display:flex;align-items:flex-end;gap:5px;">${items}</div>`
  }

  function round(num: number, match: MatchForPrint | null): string {
    ...
    return `<div style="margin-bottom:5mm;border:1px solid #ccc;">
      <div style="background:#111;padding:6px 10px;display:flex;justify-content:space-between;align-items:center;">
        <span style="color:#fff;font-weight:800;font-size:13px;letter-spacing:0.14em;">ROUND ${num}</span>
        ${filled ? '<span style="color:#f5a623;font-size:9px;font-weight:700;letter-spacing:0.04em;">PRE-FILLED — write score only</span>' : ''}
      </div>
      <div style="padding:8px 12px;">
        <div style="display:flex;justify-content:space-between;margin-bottom:7px;">
          <span style="color:#f5a623;font-weight:700;font-size:10px;letter-spacing:0.12em;">TEAM 1</span>
          <span style="color:#f5a623;font-weight:700;font-size:10px;letter-spacing:0.12em;">TEAM 2</span>
        </div>
        ...
        <div style="display:flex;align-items:center;gap:0;margin-bottom:8px;">
          ...
          <div style="width:36px;text-align:center;font-weight:800;font-size:11px;color:#555;padding-bottom:2px;">VS</div>
          <div style="flex:1;">${playerRow(b2?.player_number, b2?.naam, 'right', filled)}</div>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <div style="display:flex;align-items:center;gap:5px;">
            <span style="font-size:9px;color:#888;font-weight:600;letter-spacing:0.05em;">score</span>
            <div style="width:46px;height:30px;border:1.5px solid #333;"></div>
          </div>
  ...
  const pages = courts.map(({ courtNumber, round1 }) => `
<div style="width:100%;min-height:273mm;page-break-after:always;display:flex;flex-direction:column;">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:5mm;margin-bottom:5mm;border-bottom:1.5px solid #ccc;">
    <div style="display:flex;align-items:center;gap:10px;">
      ${paddle}
      <div style="line-height:1.1;">
        <div style="font-size:24px;font-weight:300;color:#111;letter-spacing:-0.01em;">paleta</div>
        <div style="font-size:10px;font-weight:800;color:#f5a623;letter-spacing:0.14em;margin-top:2px;">PADEL</div>
      </div>
    </div>
    <div style="font-size:30px;font-weight:900;color:#111;">Court ${courtNumber}</div>
  </div>
  ${round(1, round1)}
  ${round(2, null)}
  ${round(3, null)}
  <div style="margin-top:auto;padding-top:3mm;text-align:center;font-size:9px;color:#aaa;letter-spacing:0.1em;border-top:1px solid #eee;">paletapadel.com</div>
</div>`).join('\n')
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  body{width:186mm;margin:0 auto;padding:12mm 0;font-family:Arial,Helvetica,sans-serif;background:transparent}
  @media print{
    html{background:#fff}
...
function PlayerAssignmentCard({ match, playerOptions, onSaved }: {
  match: Props['event']['event_matches'][0]; playerOptions: PlayerOption[]; onSaved: () => void
}) {
...
// ── Score entry card ──────────────────────────────────────────────────────
function MatchCard({ match, onSaved }: { match: Props['event']['event_matches'][0]; onSaved: () => void }) {
  const [bezig, setBezig] = useState(false)
  const [fout, setFout]   = useState<string | null>(null)
...
function EventEditForm({ event, onSaved }: { event: Props['event']; onSaved: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault(); setFout(null); setBezig(true)
    const result = await updateEvent(new FormData(e.currentTarget))
    if (result?.fout) setFout(result.fout); else onSaved()
    setBezig(false)
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            ...
            <input name="datetime" type="datetime-local" required defaultValue={toLocalDatetimeInput(event.datetime)} style={fieldStyle} onFocus={focusOn} onBlur={focusOff} />
          </div>
          <div>
            <label style={editLabelStyle}>End time <span style={optStyle}>(opt.)</span></label>
            <input name="end_time" type="time" defaultValue={event.end_time?.slice(0, 5) ?? ''} style={fieldStyle} onFocus={focusOn} onBlur={focusOff} />
          </div>
        </div>
        <div>
        ...
        <div>
          <label style={editLabelStyle}>Organizer <span style={optStyle}>(opt.)</span></label>
          <input name="organizer" type="text" defaultValue={event.organizer ?? ''} style={fieldStyle} onFocus={focusOn} onBlur={focusOff} />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
...
export default function ScoreEntry({ event: initialEvent }: Props) {
  ...
  function handlePrintScoreSheets() {
    ...
    const html = buildScoreSheetHtml(courts)
  ...
  return (
    <div className="space-y-5">
      ...
      <div style={cardStyle} className="p-5">
        <EventEditForm event={event} onSaved={() => window.location.reload()} />
      </div>
        ...
        <div key={round} style={cardStyle} className="p-5">
          ...
          <div className="space-y-3">
            {rounds[round].sort((a, b) => a.court_number - b.court_number).map(m =>
              m.player_a1 === null ? (
                <PlayerAssignmentCard key={m.id} match={m} playerOptions={playerOptions} onSaved={() => window.location.reload()} />
              ) : (
                <MatchCard key={m.id} match={m} onSaved={() => window.location.reload()} />
              )
            )}
          </div>
        </div>
